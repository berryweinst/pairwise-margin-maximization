...
parser.add_argument('--save', metavar='SAVE', default='',
                    help='saved folder')
parser.add_argument('--datasets-dir', metavar='DATASETS_DIR', default='/2T_volume_sata/media/media/drive/Datasets',
                    help='datasets dir')
...
parser.add_argument('--optimize-mms', action='store_true', default=False,
                    help='whether or not to optimise radius/mms term')
parser.add_argument('--start-alpha-mms', default=1e-4, type=float,
                    help='mms regularization scale start')
parser.add_argument('--end-alpha-mms', default=1e-2, type=float,
                    help='mms regularization scale start')
...
def main_worker(args):
    ...
    model_config = {'dataset': args.dataset, 'optimize_mms': args.optimize_mms}
    ...
    trainer = Trainer(model, criterion, optimizer,
                      ...
                      optimize_mms=args.optimize_mms, mms_alpha=(args.start_alpha_mms, args.end_alpha_mms),
                      epochs = args.epochs)
    if args.tensorwatch:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        results.plot(x='epoch', y=['training sum_w_ij', 'validation sum_w_ij'],
                     legend=['training', 'validation'],
                     title='sum_w_ij', ylabel='sum_w_ij')
        results.plot(x='epoch', y=['training wt2', 'validation wt2'],
                     legend=['training', 'validation'],
                     title='wt2', ylabel='wt2')
        results.plot(x='epoch', y=['training radius', 'validation radius'],
                     legend=['training', 'validation'],
                     title='radius', ylabel='radius')
...
#--dataset cifar100 --model resnet --model-config "{'depth': 28, 'regime':'wide-resnet','width': [160, 320, 640]}" --epochs 250 --save resnet28_wide_cifar100_optimize_radius_div_mms_sqr -b 64 --device-ids 3 --autoaugment --cutout
#--dataset cifar100 --model resnet --model-config "{'depth': 28, 'regime':'wide-resnet','width': [160, 320, 640]}" --epochs 200 --save resnet28_wide_cifar100_wt2_rmax_sqr_alpha_1e_5_1e_3 -b 64 --device-ids 0 --autoaugment --cutout --epochs 200 --optimize-mms --start-alpha-mms 1e-5 --end-alpha-mms 1e-3