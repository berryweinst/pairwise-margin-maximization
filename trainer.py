...
class Trainer(object):
    ...
    def __init__(self, model, criterion, optimizer=None,
                 ...
                 optimize_mms=True, mms_alpha=None, epochs=200, batch_size=64):
        self._model = model
    ...
    # def mms_loss(self, logit):
    #
    #     topk_vals, pred_classes = logit.topk(2, 1)
    #
    #     w1 = self._model.fc.weight.index_select(0, pred_classes[:, 0])
    #     w2 = self._model.fc.weight.index_select(0, pred_classes[:, 1])
    ...
    def mms_loss(self, logit, target, mode='mean', optimize=False):
        ...
        w1 = self._model.fc.weight.index_select(0, target)
        w2 = self._model.fc.weight.index_select(0, logit_top1_indices)
    ...
    def _step(self, inputs_batch, target_batch, training=False, average_output=False, chunk_batch=1):
        ...
        for i, (inputs, target) in enumerate(zip(inputs_batch.chunk(chunk_batch, dim=0),
            ...
            # compute output
            output, mms_scaled, radius = self.model(inputs, target)
            mms_scaled = mms_scaled.sum()
            radius = radius.sum()
            # output, features = self.model(inputs, target)
            ...
            if self.optimize_mms:
                # features_list = [torch.randn(features.size()).cuda()] * 4
                # dist.all_gather(features_list, features)
                # x = torch.cat(features_list)
                #
                # radius = self.max_radious(x, optimize=True)
                # mms_scaled = torch.pow(radius * self.mms_loss(output, target, optimize=True), 2)

                mms_alpha_scale = self.mms_alpha[0] + self.epoch * (
                        (self.mms_alpha[1] - self.mms_alpha[0]) / self.epochs) if self.epoch > 0 else 0.
                loss = self.criterion(output, target) + mms_alpha_scale * mms_scaled
            else:
                loss = self.criterion(output, target)
        ...
        return outputs, total_loss, grad, 0, 0, radius


    def select_mms(self, inputs, target):
        self._model.eval()
        with torch.no_grad():
            inputs = inputs.to(self.device, dtype=self.dtype)
            target = target.to(self.device)
            logit, _, _ = self._model(inputs, target)
            topk_vals, pred_classes = logit.topk(2, 1)
            ...
            _, min_mms_indices = torch.sort(mms)
    ...
    def forward(self, data_loader, num_steps=None, training=False, average_output=False, chunk_batch=1):
        ...
        meters = {name: AverageMeter()
                  for name in ['step', 'data', 'loss', 'prec1', 'prec5', 'sum_w_ij', 'wt2', 'radius']}
        ...
        for i, (inputs, target) in enumerate(data_loader):
            ...
            # if training:
            #     inputs, target = self.select_mms(inputs, target)

            output, loss, grad, sum_w_ij, wt2, radius = self._step(inputs, target,
                                                                training=training,
            ...
            meters['sum_w_ij'].update(float(sum_w_ij), 1)
            meters['wt2'].update(float(wt2), 1)
            meters['radius'].update(float(radius), 1)
            ...
            if i % self.print_freq == 0 or i == len(data_loader) - 1:
                report = str('{phase} - Epoch: [{0}][{1}/{2}]\t'
                             ...
                             'sum_w_ij {meters[sum_w_ij].val:.3f} ({meters[sum_w_ij].avg:.3f})\t'
                             'wt2 {meters[wt2].val:.3f} ({meters[wt2].avg:.3f})\t'
                             'radius {meters[radius].val:.3f} ({meters[radius].avg:.3f})\t'
                             .format(
    ...
    def validate(self, data_loader, average_output=False):
        ...
        # if self.training_steps % (self.train_batches * 10) == 0:
        self.model.eval()
        with torch.no_grad():
