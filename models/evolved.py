...
class EvolvedNetworkCIFAR(nn.Module):
    def __init__(self, init_channels=36, num_classes=10, layers=20, auxiliary=True, aux_weight=0.4,
                 drop_path=0.2, num_epochs=600, init_lr=0.025, cell_fn=DARTSCell, optimize_mms=False):
    ...
    def mms_loss(self, logit, mode='mean', optimize=False):
    ...
    def mean_radious(self, features, optimize=False):
        radius = features.norm(dim=-1).mean()
        if not optimize:
            radius = radius.detach()
        return radius


    def logistic_mms_with_x(self, logit, features, mode='mean', optimize=False):
        topk_vals, pred_classes = logit.topk(2, 1)
        ...
        mms = (topk_vals[:, 0] - topk_vals[:, 1]) / (w1 - w2).norm(dim=-1)
        radius = features.norm(dim=-1)

        # mms = -torch.log(torch.sigmoid((mms / (radius + 1e-10)) - 10))
        # mms = -torch.log(mms / radius)
        mms = -mms / (radius + 1e-10)
        # mms = radius / (mms + 1e-10)
    ...
    def forward(self, input):
        ...
        if self.optimize_mms:
            ...
            radius = self.mean_radious(out, optimize=True)
            mms_scaled = radius / self.mms_loss(logits, optimize=True)
            # mms_scaled = self.mms_loss(x, optimize=True) / radius
            # mms_scaled, radius = self.logistic_mms_with_x(x, features, optimize=True)
        else:
            with torch.no_grad():
                radius = self.max_radious(out, optimize=False)
                mms_scaled = radius / self.mms_loss(logits, optimize=False)


        if self._auxiliary:
            return (logits, logits_aux), mms_scaled, radius
        else:
            return logits, mms_scaled, radius
